...
case ${VERSION} in
  ...
  "24.04")
    if [ ! -f "ubuntu-2404-base-amd64.tar.gz" ]; then
      wget -O ubuntu-2404-base-amd64.tar.gz https://cdimage.ubuntu.com/ubuntu-base/releases/24.04/release/ubuntu-base-24.04.2-base-amd64.tar.gz
    fi
...
apt install -qy --no-install-recommends ${pkg_install} grub-common grub-efi-amd64 grub-pc-bin systemd initramfs-tools \
	...
	rsyslog bash-completion cloud-init cloud-initramfs-growroot cloud-utils
